...
def next(roomId = "room01"):
    ...
    # length = randint(2, 5)
    length = choice([2] * 2 + [3] * 2 + [4] * 2 + [5] * 1)
    while True:
        word = choice(WordDict[str(length)])
        if word not in Room.wordMap:
            break
...
def finish(roomId = "room01"):
    ...
    scores = [[rank, user, score, int(score / TOTAL * 100)] \
        for rank, (user, score) in enumerate(scores, start=1)]
    
    print(f"{Room.tries} tries")
    print(f"total {TOTAL} words removed")
    for rank, user, score, contr in scores:
        print(f"rank {rank}: {user}, contributed: {score}({contr}%)")
